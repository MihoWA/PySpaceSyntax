...
def getjunctions(ways):
    ...
    Nroads=len(ways)
    junctions=[]
    #list of connections for each junction what directs to where...
...
def make_segments(coords, ways, intersections, arcangle):
...
def do_integration(mapname, axlines, segments, links):
    import numpy as np
...
if __name__=="__main__":
    ...
    links, axlines, segments = make_segments(coords, ways, junctions, arcangle)
    axlines=do_integration(mapname, axlines, segments, links)
    plotmap(mapname, axlines, segments, coords)
